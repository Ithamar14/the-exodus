...
public sealed class GameWorld
{
    ...
    private const float CollisionLaunchDistance = 224f;
    private const float MannaBlinkStartSeconds = 5f;
    private const float MannaLifetimeSeconds = 10f;
    private const float MannaPhaseLeadSeconds = 1f;
    private const int RequiredMannaPerPlayer = 1;
    ...
    {
        player.VelocityX = player.InputDirX * WalkSpeed;
    ...
    private void ResolveCollisions(ICollection<GameEventSnapshot> events)
    {
        ...
        for (var i = 0; i < alivePlayers.Length; i++)
        {
            for (var j = i + 1; j < alivePlayers.Length; j++)
            {
                ...
                var separation = Math.Max(overlap + _rules.CollisionBumpDistance, CollisionLaunchDistance);
    ...
    private void AdvanceManna(float deltaSeconds, ICollection<GameEventSnapshot> events)
    {
        ...
        if (!_mannaCycle.BlinkStarted && _mannaCycle.AgeSeconds >= MannaBlinkStartSeconds)
        {
            ...
            events.Add(new GameEventSnapshot(
                ...
                IsActive: true,
                SecondsUntilStateChange: Math.Max(0f, MannaLifetimeSeconds - _mannaCycle.AgeSeconds)));
        }
        ...
        if (_mannaCycle.RemainingPickupCount <= 0 || _mannaCycle.AgeSeconds >= MannaLifetimeSeconds)
        {
    ...
    private void Kill(PlayerState player, string reason, ICollection<GameEventSnapshot> events)
    {
        ...
        if (player.Lives > 0)
        {
            player.InvincibilityRemaining = 1.0f;
            events.Add(new GameEventSnapshot(
                "player_lost_life",
                PlayerId: player.Id,
                Reason: reason,
    ...
    private void SpawnMannaCycle(ICollection<GameEventSnapshot> events)
    {
        ...
        events.Add(new GameEventSnapshot(
            ...
            RemainingCount: _mannaCycle.RemainingPickupCount,
            IsActive: true,
            SecondsUntilStateChange: MannaBlinkStartSeconds));
    }
    ...
    private MannaSnapshot BuildMannaSnapshot()
    {
        if (_mannaCycle is null)
        {
            var secondsUntilNextCycle = _phase == RoundPhase.Active && _nextHazard == HazardType.Manna
                ...
                : 0f;
            return new MannaSnapshot(false, 0, secondsUntilNextCycle, 0f, 0f, false, RequiredMannaPerPlayer, 0, Array.Empty<MannaPickupSnapshot>());
        }
        ...
        var pickups = _mannaCycle.Pickups
            ...
            .ToArray();
        var secondsUntilBlink = Math.Max(0f, MannaBlinkStartSeconds - _mannaCycle.AgeSeconds);
        var secondsUntilDisappear = Math.Max(0f, MannaLifetimeSeconds - _mannaCycle.AgeSeconds);
        ...
        return new MannaSnapshot(
            ...
            _mannaCycle.AgeSeconds >= MannaBlinkStartSeconds,
            RequiredMannaPerPlayer,
    ...
    private void PickNextHazard(bool applyTransitionDelay)
    {
        ...
        _mannaSpawnDelayRemaining = MannaPhaseLeadSeconds + transitionDelay;
    }
